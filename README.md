...
```json
"@zero/dayline": "github:L0RIZI0/zero-dayline#v0.5.0"
```
...
Sky: `data.sky = { sun?: boolean, moon?: boolean }` — sun defaults on, moon off. Moon is a bluish sinusoid that **crosses the axis at moonrise / moonset** (same knot rule as the sun; ~50 min later each day). Hover the curve for a crescent + rise/set times.
...
Releases are **manual**. GitHub Actions → *release* → Run workflow → version `0.5.0` (no `v`). That type-checks and pushes tag `v0.5.0`.
